...
def init_conversations():
    ...
    create_tables()
    conversations = list(
        reversed(get_all_conversations(st.session_state.user_id)))
    conversation_options = [
        f"{conversation['name']}" for conversation in conversations]
...
def conversation_navigation():
    ...
    with st.expander(label="Conversation Histories", expanded=False):
...
def create_conversation(conversation_options):
    ...
    if st.button("Add New Conversation") and new_conversation_name.strip() != "":
        if new_conversation_name in conversation_options:
            st.warning(
                "Conversation with that name already exists. Please choose a different name.")
        else:
            conversation_id = str(uuid.uuid4())
            user_id = st.session_state.user_id
            new_conversation = Conversation(
                conversation_id, user_id, new_conversation_name)
            save_conversation(new_conversation)
            st.success(
                f"Conversation '{new_conversation_name}' added successfully!")
            st.rerun()


def navigate_past_conversations(conversations, conversation_options):
    if len(conversation_options) > 0:
        icons_conversations = ['chat-right-dots-fill'] * \
            len(conversation_options)
