...
def dashboard3():
    ...
    def calculate_leak_probability(sound_strength):
        normalized_strength = (sound_strength - sound_strength.min()) / (sound_strength.max() - sound_strength.min())
        return 1 / (1 + np.exp(-10 * (normalized_strength - 0.5)))
    ...
    if df_selected_date.empty or 'latitude' not in df.columns or 'longitude' not in df.columns:
        st.warning("There is no data on the selected date or necessary columns are missing.")
        return

    def make_choropleth(input_df, color):
        if input_df.empty or 'latitude' not in input_df.columns or 'longitude' not in input_df.columns:
            st.warning('There is no data available for the choropleth map.')
            return None
    ...
    with col1:
        ...
        sound_strength_chart = alt.Chart(df_selected_date).mark_bar().encode(
            ...
            tooltip=['ID:O', 'Sound_strength:Q', 'AI_result:N']
    ...
    with col1:
        ...
        df_selected_date = df_selected_date[df_selected_date['facil_type'] == 'Pipe']
        ...
        fig_facility_type = px.scatter_mapbox(
            ...
            hover_data={"latitude": True, "longitude": True, "ID": True, 
    ...
    with col2:
        facility_type = 'Pipe'
        icon_html = '<i class="fas fa-tint pipe-icon"></i>'  
        class_name = 'pipe-text'
        background_color = '#fce4d6'

        count = df_selected_date['facil_type'].value_counts().get(facility_type, 0)
        
        st.markdown(f"""
        <div class="card" style="background-color: {background_color};">
            <h3 class="{class_name}">{facility_type} {icon_html}</h3>
    ...
    if not leak_df.empty:
        ...
        leak_df_subset = leak_df[
            ['Alarm', 'ID', 'latitude', 'longitude', 'AI_result', 'Sound_strength', 'leak_probability', 'recording_time']]
